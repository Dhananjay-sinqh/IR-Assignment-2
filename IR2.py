import math
def trans(X, Y):
    result = [[0, 0, 0], [0, 0, 0], [0, 0, 0]] #initial matrix final edit
...
def change():
    ...
    theta = list(
        map(
            int,
            input(
                "The angles of rotation about the x y z coordinates: "
            )
            .strip()
            .split(),
        )
    ...
    print(
        f"Coordinates of Point {coord[0], coord[1], coord[2]} with respect to original frame is {round(final_result[0], 3), round(final_result[1], 3), round(final_result[2], 3)}"
    )
